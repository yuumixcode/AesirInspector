...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class CustomValueDrawerAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData("用一个方法代替「新建特性 + 新建 Drawer」的流程，方法充当该字段的自定义绘制器；绘制器自带撤销 / 重做与多选支持。", "Replaces the \"new attribute + new drawer\" workflow with a single method that acts as the field's custom drawer; the drawer supports undo / redo and multi-selection out of the box."),
            new BilingualData("Action 支持 $ 成员引用与 @ 表达式；方法通常接收 (T value, GUIContent label) 并返回 T，也可只接收 value 或额外接收 callNextDrawer。", "Action supports $ member references and @ expressions; the method usually takes (T value, GUIContent label) and returns T, but may also take only value or additionally a callNextDrawer."),
            new BilingualData("方法的返回值会写回字段；在方法内调用 callNextDrawer(label) 可先绘制默认绘制器。", "The method's return value is written back to the field; calling callNextDrawer(label) inside the method first draws the default drawer.")
        };
