...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class ValueDropdownAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData(
                "为属性提供自定义下拉列表：valuesGetter 指向返回 IList 的成员或表达式（数组、List、ValueDropdownList<T> 均可）。",
                "Provides a custom dropdown for a property: valuesGetter points to a member or expression returning an IList (arrays, Lists and ValueDropdownList<T> all work)."),
            new BilingualData(
                "选项文本用 \"Group/Item\" 形式即可形成树状分组，配合 ExpandAllMenuItems 默认展开；选项达到 10 个时会启用搜索框（设为 0 则始终启用）。",
            ...
            new BilingualData(
                "作用于列表时默认也为每个元素提供下拉（DrawDropdownForListElements）；IsUniqueList 保证元素唯一并启用多选，ExcludeExistingValuesInList 可隐藏已选项。",
        ...
        {
            new ParameterValue(typeof(string).FullName, "valuesGetter",
        ...
        {
            new ResolvedStringParameterValue("ValuesGetter", ResolverType.ValueResolver,
