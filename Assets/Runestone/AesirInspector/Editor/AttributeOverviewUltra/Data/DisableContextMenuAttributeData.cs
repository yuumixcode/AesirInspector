...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class DisableContextMenuAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData(
                "默认禁用成员自身的右键菜单（disableForMember = true）；disableCollectionElements 为 true 时可同时禁用集合元素的右键菜单。",
                "Disables the member's own context menu by default (disableForMember = true); setting disableCollectionElements to true also disables the context menus of collection elements."),
            new BilingualData("只禁用 Odin 提供的右键菜单，不会禁用 Unity 自带对象字段的右键菜单。",
                "It only disables the context menus provided by Odin; Unity's own object-field context menu is not disabled.")
        };
