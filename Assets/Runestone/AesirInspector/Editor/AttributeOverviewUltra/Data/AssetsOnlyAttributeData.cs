namespace Runestone.AesirInspector.Editor
{
    ...
    internal class AssetsOnlyAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData("把 UnityEngine.Object 字段限制为项目资源（预制体、材质、网格等），场景中的对象无法被赋值。",
                "Restricts a UnityEngine.Object field to project assets (prefabs, materials, meshes, and so on); objects from the scene cannot be assigned."),
            new BilingualData("该特性没有参数，可用于单个对象字段，也可用于对象列表或数组。",
                "The attribute has no parameters and works on a single object field as well as on lists or arrays of objects."),
            new BilingualData("与 SceneObjectsOnly 相反：当需要确保对象来自场景而非项目资源时，使用 SceneObjectsOnly。",
                "It is the opposite of SceneObjectsOnly: use SceneObjectsOnly when you must ensure the object comes from the scene rather than the project.")
        };
