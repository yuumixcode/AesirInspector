...
namespace Runestone.AesirInspector.Editor
{
    internal class OnCollectionChangedAttributeData : AbstractAttributeData
    {
        ...
        public override BilingualData[] UsageTips { get; set; } =
        {
            ...
            new BilingualData(
                "适用于任何具备集合解析器的集合，如数组、List、Dictionary、HashSet、Stack、LinkedList（Dictionary/HashSet 需要 Odin 序列化）。",
            ...
            new BilingualData(
                "回调方法可接收 CollectionChangeInfo 获取变更详情；它是仅编辑器可用的结构体（命名空间 Sirenix.OdinInspector.Editor），必须放在 #if UNITY_EDITOR 中。",
