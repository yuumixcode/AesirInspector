...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class EnableIfAttributeData : AbstractAttributeData
    {
        ...
        public override BilingualData[] UsageTips { get; set; } =
        {
            ...
            new BilingualData("条件不满足时属性仍然绘制，只是被禁用（灰显），其值依然会被序列化。",
                "When the condition is not met the property is still drawn, only disabled (grayed out), and its value is still serialized."),
            new BilingualData("与 DisableIf 相反；当 condition 无法解析时，属性默认为启用状态。",
                "It is the opposite of DisableIf; when condition cannot be resolved, the property defaults to the enabled state.")
        };
