...
namespace Runestone.AesirInspector.Editor
{
    internal class EnableInAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData("只有当属性所在对象的 PrefabKind 与参数位掩码有交集时才会启用；无交集（包括 PrefabKind.None）时属性被禁用。",
                "The property is enabled only when the PrefabKind of its object intersects the parameter bitmask; with no intersection (including PrefabKind.None) it is disabled."),
            new BilingualData(
                "PrefabKind 由目标对象解析，因此只对 GameObject 或 Component 上的成员有意义；ScriptableObject 等对象始终视为 PrefabKind.None，属性始终禁用。",
                "The PrefabKind is resolved from the target object, so it only makes sense for members on a GameObject or Component; other objects such as ScriptableObjects always resolve to PrefabKind.None, leaving the property disabled."),
            new BilingualData("与 DisableIn 相反：EnableIn 是白名单（默认禁用，匹配时才启用），DisableIn 是黑名单（默认可用，匹配时才禁用）。",
                "It is the opposite of DisableIn: EnableIn is a whitelist (disabled by default, enabled on a match) while DisableIn is a blacklist (usable by default, disabled on a match)."),
            new BilingualData("标注在集合上时作用于整个集合，不会逐个作用于列表元素。",
                "Applied to a collection it affects the collection as a whole rather than each list element.")
        };
