...
namespace Runestone.AesirInspector.Editor
{
    internal class DisableInAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData("只有当属性所在对象的 PrefabKind 与参数位掩码有交集时才会被禁用，无交集时保持可编辑。",
                "The property is disabled only when the PrefabKind of its object intersects the parameter bitmask; with no intersection it stays editable."),
            new BilingualData(
                "PrefabKind 由目标对象解析，因此只对 GameObject 或 Component 上的成员有意义；ScriptableObject 等对象始终视为 PrefabKind.None。",
                "The PrefabKind is resolved from the target object, so it only makes sense for members on a GameObject or Component; other objects such as ScriptableObjects always resolve to PrefabKind.None."),
            new BilingualData("被禁用后属性仍然绘制，只是失去焦点、无法修改；HideIn 则是直接隐藏属性。",
                "A disabled property is still drawn, but cannot be focused or modified; HideIn hides it outright instead."),
