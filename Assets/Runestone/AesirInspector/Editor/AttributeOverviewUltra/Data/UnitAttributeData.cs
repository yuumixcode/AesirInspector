...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class UnitAttributeData : AbstractAttributeData
    {
        public override BilingualHeaderControl BilingualHeaderControl { get; set; } =
            new BilingualHeaderControl("Unit", "Unit", "Unit 特性为数值字段添加单位，并支持在基准单位与显示单位之间换算，让数值更符合使用习惯。",
        ...
        public override ParameterValue[] AttributeParameters { get; set; } =
        {
            ...
            new ParameterValue("Units", "base",
                new BilingualData("基准单位，字段值以该单位存储。", "The base unit in which the field value is stored.")),
            new ParameterValue("Units", "display",
                new BilingualData("显示单位，绘制时按该单位换算。", "The display unit used when drawing the value.")),
            new ParameterValue(typeof(bool).FullName, "DisplayAsString",
                new BilingualData("是否以只读文本显示（不可编辑）。", "Whether to display the value as read-only text.")),
        ...
        {
            new AttributeExamplePreviewItem().InitializeUnitySerializedExample("Unit", UnitExampleSO.Instance)
        };
    }
}
