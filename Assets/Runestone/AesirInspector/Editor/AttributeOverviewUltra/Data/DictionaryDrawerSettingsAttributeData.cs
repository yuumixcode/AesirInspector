...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class DictionaryDrawerSettingsAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData("用于自定义字典在 Inspector 中的绘制；字典必须使用 Odin 序列化（[OdinSerialize]）才能在检视器中编辑。",
                "Customizes how a dictionary is drawn in the Inspector; the dictionary must use Odin serialization ([OdinSerialize]) to be editable there."),
            new BilingualData(
                "KeyLabel / ValueLabel 自定义键值列标签（默认 \"Key\" / \"Value\"），KeyColumnWidth 设置键列宽度（默认 130）。",
                "KeyLabel / ValueLabel override the key and value column labels (default \"Key\" / \"Value\"), and KeyColumnWidth sets the key column width (default 130)."),
            new BilingualData(
                "DisplayMode 控制绘制方式（OneLine、Foldout、CollapsedFoldout、ExpandedFoldout），IsReadOnly 可禁止在面板中增删条目。",
                "DisplayMode controls the drawing style (OneLine, Foldout, CollapsedFoldout, ExpandedFoldout), and IsReadOnly prevents adding or removing entries in the Inspector.")
        };
