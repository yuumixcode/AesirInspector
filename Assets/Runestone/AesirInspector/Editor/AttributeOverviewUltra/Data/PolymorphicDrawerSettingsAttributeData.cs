...
namespace Runestone.AesirInspector.Editor
{
    internal class PolymorphicDrawerSettingsAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData(
                "用于配置由 Odin 绘制的多态字段（接口、抽象类）；Unity 默认无法序列化这类类型，需要 Odin 序列化（如 SerializedMonoBehaviour 或 [OdinSerialize]）。",
            ...
            new BilingualData(
                "CreateInstanceFunction 需指向一个接收单个 Type 参数（名为 type）并返回 object 的方法，且不会用于 UnityEngine.Object 类型；未指定时由 NonDefaultConstructorPreference（默认 ConstructIdeal）决定实例构造方式。",
