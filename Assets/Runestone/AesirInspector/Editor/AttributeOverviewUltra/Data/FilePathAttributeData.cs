namespace Runestone.AesirInspector.Editor
{
    ...
    internal class FilePathAttributeData : AbstractAttributeData
    {
        ...
        public override BilingualData[] UsageTips { get; set; } =
        {
            ...
            new BilingualData(
                "ParentFolder 与 Extensions 都支持 $ 成员引用（例如 ParentFolder = \"$DynamicParent\"），可以动态改变相对基准目录与允许的扩展名。",
                "Both ParentFolder and Extensions support $ member references (for example ParentFolder = \"$DynamicParent\"), so the base folder and allowed extensions can change dynamically."),
            new BilingualData("支持从 Project 窗口直接拖放文件到该字段，也可以用于 string[] 等集合的元素。",
                "Files can be dragged and dropped from the Project window onto the field, and the attribute also works on elements of collections such as string[]."),
            new BilingualData("RequireExistingPath 为 true 时，路径不存在会报校验错误。",
                "With RequireExistingPath set to true, a non-existing path raises a validation error.")
        };
