    ...
    internal class OnValueChangedAttributeData : AbstractAttributeData
    {
        ...
        public override ParameterValue[] AttributeParameters { get; set; } =
        {
            new ParameterValue(typeof(string).FullName, "action",
                new BilingualData("值改变时执行的操作（解析字符串：方法名、表达式或方法调用）。",
                    "The action to perform when the value changes (a resolved string: method name, expression or method invocation).")),
