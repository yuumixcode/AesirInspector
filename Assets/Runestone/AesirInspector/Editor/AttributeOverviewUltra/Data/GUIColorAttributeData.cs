...
namespace Runestone.AesirInspector.Editor
{
    ...
    internal class GUIColorAttributeData : AbstractAttributeData
    {
        ...
        {
            new BilingualData("可以用 r、g、b、a 直接指定颜色（三参数构造重载中 a 默认为 1），也可以用 getColor 以字符串解析颜色。",
                "The color can be specified directly with r, g, b and a (the three-argument constructor overload defaults a to 1), or resolved from a string with getColor."),
            new BilingualData(
                "getColor 支持 $ 成员引用与 @ 表达式，例如 \"@useRed ? UnityEngine.Color.red : UnityEngine.Color.green\" 可根据条件动态改变颜色。",
                "getColor supports $ member references and @ expressions, so for example \"@useRed ? UnityEngine.Color.red : UnityEngine.Color.green\" changes the color dynamically based on a condition."),
            new BilingualData("颜色作用于整个属性的绘制（包含标签与按钮），因此常用来突出重要字段或按钮。",
                "The color applies to the drawing of the whole property, including its label and buttons, which makes it handy for highlighting important fields or buttons.")
        };
