...
namespace Sirenix.Utilities.Editor
{
	...
	{
		/// <summary>
		/// Returns <c>true</c> when the users mouse is hovering over the specified <see cref="T:UnityEngine.Rect" />
		/// </summary>
		/// <param name="current">The  <see cref="T:UnityEngine.Event" />.</param>
		/// <param name="rect">The <see cref="T:UnityEngine.Rect" /> to check.</param>
		/// <returns><c>true</c> if the mouse is over the specified <see cref="T:UnityEngine.Rect" /> otherwise <c>false</c>.</returns>
		...
		{
			bool result = current.type == EventType.MouseDown && current.button == mouseButton;
			if (result && useEvent)
			{
				current.Use();
			}
		...
		{
			bool result = current.type == EventType.MouseUp && current.button == mouseButton;
			if (result && useEvent)
			{
				current.Use();
			}
		...
		{
			return current.OnMouseDown(rect, 0, useEvent);
		}
		...
		{
			bool result = current.type == EventType.ContextClick && rect.Contains(current.mousePosition);
			if (result && useEvent)
			{
				current.Use();
			}
		...
		{
			bool result = current.type == EventType.KeyDown && current.keyCode == key;
			if (result && useEvent)
			{
				current.Use();
			}
		...
		{
			bool result = current.type == EventType.KeyUp && current.keyCode == key;
			if (result && useEvent)
			{
				current.Use();
			}
		...
		{
			bool result = current.type == EventType.MouseMove || current.type == EventType.MouseDrag;
			if (result && useEvent)
			{
				current.Use();
			}
