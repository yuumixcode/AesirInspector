using System;
using System.ComponentModel;
using Sirenix.OdinInspector;
using Sirenix.OdinInspector.Editor;
using UnityEditor;
using UnityEngine;

namespace Sirenix.Utilities.Editor
{
	/// <summary>
	/// Collection of EditorIcons for use in GUI drawing.
	/// </summary>
	public static class EditorIcons
	{
		private static Texture2D testInconclusive;

		private static Texture2D testFailed;

		private static Texture2D testNormal;

		private static Texture2D testPassed;

		private static Texture2D consoleInfoicon;

		private static Texture2D consoleWarnicon;

		private static Texture2D consoleErroricon;

		private static Texture2D unityInfoIcon;

		private static Texture2D unityErrorIcon;

		private static Texture2D unityWarningIcon;

		private static Texture2D unityFolderIcon;

		private static Texture2D odinInspectorLogo;

		private static Texture2D unityLogo;

		private static LazyEditorIcon airplane;

		private static LazyEditorIcon alertCircle;

		private static LazyEditorIcon alertTriangle;

		private static LazyEditorIcon arrowDown;

		private static LazyEditorIcon arrowLeft;

		private static LazyEditorIcon arrowRight;

		private static LazyEditorIcon arrowUp;

		private static LazyEditorIcon bell;

		private static LazyEditorIcon car;

		private static LazyEditorIcon char1;

		private static LazyEditorIcon char2;

		private static LazyEditorIcon char3;

		private static LazyEditorIcon charGraph;

		private static LazyEditorIcon checkmark;

		private static LazyEditorIcon clock;

		private static LazyEditorIcon clouds;

		private static LazyEditorIcon cloudsRainy;

		private static LazyEditorIcon cloudsRainySunny;

		private static LazyEditorIcon cloudsRainyThunder;

		private static LazyEditorIcon cloudsThunder;

		private static LazyEditorIcon crosshair;

		private static LazyEditorIcon cut;

		private static LazyEditorIcon dayCalendar;

		private static LazyEditorIcon download;

		private static LazyEditorIcon eject;

		private static LazyEditorIcon eyeDropper;

		private static LazyEditorIcon female;

		private static LazyEditorIcon file;

		private static LazyEditorIcon fileCabinet;

		private static LazyEditorIcon finnishBanner;

		private static LazyEditorIcon flag;

		private static LazyEditorIcon flagFinnish;

		private static LazyEditorIcon folder;

		private static LazyEditorIcon folderBack;

		private static LazyEditorIcon gKey;

		private static LazyEditorIcon globe;

		private static LazyEditorIcon gridBlocks;

		private static LazyEditorIcon gridImageText;

		private static LazyEditorIcon gridImageTextList;

		private static LazyEditorIcon gridLayout;

		private static LazyEditorIcon hamburgerMenu;

		private static LazyEditorIcon house;

		private static LazyEditorIcon image;

		private static LazyEditorIcon imageCollection;

		private static LazyEditorIcon info;

		private static LazyEditorIcon letter;

		private static LazyEditorIcon lightBulb;

		private static LazyEditorIcon link;

		private static LazyEditorIcon list;

		private static LazyEditorIcon loadingBar;

		private static LazyEditorIcon lockLocked;

		private static LazyEditorIcon lockUnlocked;

		private static LazyEditorIcon magnifyingGlass;

		private static LazyEditorIcon male;

		private static LazyEditorIcon marker;

		private static LazyEditorIcon maximize;

		private static LazyEditorIcon microphone;

		private static LazyEditorIcon minimize;

		private static LazyEditorIcon minus;

		private static LazyEditorIcon mobilePhone;

		private static LazyEditorIcon money;

		private static LazyEditorIcon move;

		private static LazyEditorIcon multiUser;

		private static LazyEditorIcon next;

		private static LazyEditorIcon pacmanGhost;

		private static LazyEditorIcon paperclip;

		private static LazyEditorIcon pause;

		private static LazyEditorIcon pen;

		private static LazyEditorIcon penAdd;

		private static LazyEditorIcon penMinus;

		private static LazyEditorIcon play;

		private static LazyEditorIcon plus;

		private static LazyEditorIcon podium;

		private static LazyEditorIcon previous;

		private static LazyEditorIcon receptionSignal;

		private static LazyEditorIcon redo;

		private static LazyEditorIcon refresh;

		private static LazyEditorIcon rotate;

		private static LazyEditorIcon ruler;

		private static LazyEditorIcon rulerRect;

		private static LazyEditorIcon settingsCog;

		private static LazyEditorIcon shoppingBasket;

		private static LazyEditorIcon shoppingCart;

		private static LazyEditorIcon singleUser;

		private static LazyEditorIcon smartPhone;

		private static LazyEditorIcon sound;

		private static LazyEditorIcon speechBubbleRound;

		private static LazyEditorIcon speechBubbleSquare;

		private static LazyEditorIcon speechBubblesRound;

		private static LazyEditorIcon speechBubblesSquare;

		private static LazyEditorIcon starPointer;

		private static LazyEditorIcon stop;

		private static LazyEditorIcon stretch;

		private static LazyEditorIcon table;

		private static LazyEditorIcon tag;

		private static LazyEditorIcon testTube;

		private static LazyEditorIcon timer;

		private static LazyEditorIcon trafficStopLight;

		private static LazyEditorIcon transparent;

		private static LazyEditorIcon tree;

		private static LazyEditorIcon triangleDown;

		private static LazyEditorIcon triangleLeft;

		private static LazyEditorIcon triangleRight;

		private static LazyEditorIcon triangleUp;

		private static LazyEditorIcon undo;

		private static LazyEditorIcon upload;

		private static LazyEditorIcon wifiSignal;

		private static LazyEditorIcon x;

		private static Texture2D unityGameObjectIcon;

		private static Texture2D objectFieldButtonDark;

		private static Texture2D objectFieldButtonLight;

		/// <summary>
		/// Gets an icon of an airplane symbol.
		/// </summary>
		public static EditorIcon Airplane
		{
			get
			{
				if (airplane == null)
				{
					airplane = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(airplane);
				}
				return airplane;
			}
		}

		/// <summary>
		/// Gets an icon of an alert circle symbol.
		/// </summary>
		public static EditorIcon AlertCircle
		{
			get
			{
				if (alertCircle == null)
				{
					alertCircle = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(alertCircle);
				}
				return alertCircle;
			}
		}

		/// <summary>
		/// Gets an icon of an alert triangle symbol.
		/// </summary>
		public static EditorIcon AlertTriangle
		{
			get
			{
				if (alertTriangle == null)
				{
					alertTriangle = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(alertTriangle);
				}
				return alertTriangle;
			}
		}

		/// <summary>
		/// Gets an icon of an arrow down symbol.
		/// </summary>
		public static EditorIcon ArrowDown
		{
			get
			{
				if (arrowDown == null)
				{
					arrowDown = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(arrowDown);
				}
				return arrowDown;
			}
		}

		/// <summary>
		/// Gets an icon of an arrow left symbol.
		/// </summary>
		public static EditorIcon ArrowLeft
		{
			get
			{
				if (arrowLeft == null)
				{
					arrowLeft = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(arrowLeft);
				}
				return arrowLeft;
			}
		}

		/// <summary>
		/// Gets an icon of an arrow right symbol.
		/// </summary>
		public static EditorIcon ArrowRight
		{
			get
			{
				if (arrowRight == null)
				{
					arrowRight = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(arrowRight);
				}
				return arrowRight;
			}
		}

		/// <summary>
		/// Gets an icon of an arrow up symbol.
		/// </summary>
		public static EditorIcon ArrowUp
		{
			get
			{
				if (arrowUp == null)
				{
					arrowUp = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(arrowUp);
				}
				return arrowUp;
			}
		}

		/// <summary>
		/// Gets an icon of a bell symbol.
		/// </summary>
		public static EditorIcon Bell
		{
			get
			{
				if (bell == null)
				{
					bell = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(bell);
				}
				return bell;
			}
		}

		/// <summary>
		/// Gets an icon of a car symbol.
		/// </summary>
		public static EditorIcon Car
		{
			get
			{
				if (car == null)
				{
					car = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(car);
				}
				return car;
			}
		}

		/// <summary>
		/// Gets an icon of a char1 symbol.
		/// </summary>
		public static EditorIcon Char1
		{
			get
			{
				if (char1 == null)
				{
					char1 = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(char1);
				}
				return char1;
			}
		}

		/// <summary>
		/// Gets an icon of a char2 symbol.
		/// </summary>
		public static EditorIcon Char2
		{
			get
			{
				if (char2 == null)
				{
					char2 = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(char2);
				}
				return char2;
			}
		}

		/// <summary>
		/// Gets an icon of a char3 symbol.
		/// </summary>
		public static EditorIcon Char3
		{
			get
			{
				if (char3 == null)
				{
					char3 = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(char3);
				}
				return char3;
			}
		}

		/// <summary>
		/// Gets an icon of a char graph symbol.
		/// </summary>
		public static EditorIcon CharGraph
		{
			get
			{
				if (charGraph == null)
				{
					charGraph = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(charGraph);
				}
				return charGraph;
			}
		}

		/// <summary>
		/// Gets an icon of a checkmark symbol.
		/// </summary>
		public static EditorIcon Checkmark
		{
			get
			{
				if (checkmark == null)
				{
					checkmark = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(checkmark);
				}
				return checkmark;
			}
		}

		/// <summary>
		/// Gets an icon of a clock symbol.
		/// </summary>
		public static EditorIcon Clock
		{
			get
			{
				if (clock == null)
				{
					clock = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(clock);
				}
				return clock;
			}
		}

		/// <summary>
		/// Gets an icon of a clouds symbol.
		/// </summary>
		public static EditorIcon Clouds
		{
			get
			{
				if (clouds == null)
				{
					clouds = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(clouds);
				}
				return clouds;
			}
		}

		/// <summary>
		/// Gets an icon of a clouds rainy symbol.
		/// </summary>
		public static EditorIcon CloudsRainy
		{
			get
			{
				if (cloudsRainy == null)
				{
					cloudsRainy = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(cloudsRainy);
				}
				return cloudsRainy;
			}
		}

		/// <summary>
		/// Gets an icon of a clouds rainy sunny symbol.
		/// </summary>
		public static EditorIcon CloudsRainySunny
		{
			get
			{
				if (cloudsRainySunny == null)
				{
					cloudsRainySunny = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(cloudsRainySunny);
				}
				return cloudsRainySunny;
			}
		}

		/// <summary>
		/// Gets an icon of a clouds rainy thunder symbol.
		/// </summary>
		public static EditorIcon CloudsRainyThunder
		{
			get
			{
				if (cloudsRainyThunder == null)
				{
					cloudsRainyThunder = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(cloudsRainyThunder);
				}
				return cloudsRainyThunder;
			}
		}

		/// <summary>
		/// Gets an icon of a clouds thunder symbol.
		/// </summary>
		public static EditorIcon CloudsThunder
		{
			get
			{
				if (cloudsThunder == null)
				{
					cloudsThunder = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(cloudsThunder);
				}
				return cloudsThunder;
			}
		}

		/// <summary>
		/// Gets an icon of a crosshair symbol.
		/// </summary>
		public static EditorIcon Crosshair
		{
			get
			{
				if (crosshair == null)
				{
					crosshair = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(crosshair);
				}
				return crosshair;
			}
		}

		/// <summary>
		/// Gets an icon of a cut symbol.
		/// </summary>
		public static EditorIcon Cut
		{
			get
			{
				if (cut == null)
				{
					cut = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(cut);
				}
				return cut;
			}
		}

		/// <summary>
		/// Gets an icon of a day calendar symbol.
		/// </summary>
		public static EditorIcon DayCalendar
		{
			get
			{
				if (dayCalendar == null)
				{
					dayCalendar = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(dayCalendar);
				}
				return dayCalendar;
			}
		}

		/// <summary>
		/// Gets an icon of a download symbol.
		/// </summary>
		public static EditorIcon Download
		{
			get
			{
				if (download == null)
				{
					download = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(download);
				}
				return download;
			}
		}

		/// <summary>
		/// Gets an icon of an eject symbol.
		/// </summary>
		public static EditorIcon Eject
		{
			get
			{
				if (eject == null)
				{
					eject = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(eject);
				}
				return eject;
			}
		}

		/// <summary>
		/// Gets an icon of an eye dropper symbol.
		/// </summary>
		public static EditorIcon EyeDropper
		{
			get
			{
				if (eyeDropper == null)
				{
					eyeDropper = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(eyeDropper);
				}
				return eyeDropper;
			}
		}

		/// <summary>
		/// Gets an icon of a female symbol.
		/// </summary>
		public static EditorIcon Female
		{
			get
			{
				if (female == null)
				{
					female = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(female);
				}
				return female;
			}
		}

		/// <summary>
		/// Gets an icon of a file symbol.
		/// </summary>
		public static EditorIcon File
		{
			get
			{
				if (file == null)
				{
					file = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(file);
				}
				return file;
			}
		}

		/// <summary>
		/// Gets an icon of a file cabinet symbol.
		/// </summary>
		public static EditorIcon FileCabinet
		{
			get
			{
				if (fileCabinet == null)
				{
					fileCabinet = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(fileCabinet);
				}
				return fileCabinet;
			}
		}

		/// <summary>
		/// Gets an icon of a finnish banner symbol.
		/// </summary>
		public static EditorIcon FinnishBanner
		{
			get
			{
				if (finnishBanner == null)
				{
					finnishBanner = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(finnishBanner);
				}
				return finnishBanner;
			}
		}

		/// <summary>
		/// Gets an icon of a flag symbol.
		/// </summary>
		public static EditorIcon Flag
		{
			get
			{
				if (flag == null)
				{
					flag = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(flag);
				}
				return flag;
			}
		}

		/// <summary>
		/// Gets an icon of a flag finnish symbol.
		/// </summary>
		public static EditorIcon FlagFinnish
		{
			get
			{
				if (flagFinnish == null)
				{
					flagFinnish = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(flagFinnish);
				}
				return flagFinnish;
			}
		}

		/// <summary>
		/// Gets an icon of a folder symbol.
		/// </summary>
		public static EditorIcon Folder
		{
			get
			{
				if (folder == null)
				{
					folder = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(folder);
				}
				return folder;
			}
		}

		/// <summary>
		/// Gets an icon of a folder back symbol.
		/// </summary>
		public static EditorIcon FolderBack
		{
			get
			{
				if (folderBack == null)
				{
					folderBack = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(folderBack);
				}
				return folderBack;
			}
		}

		/// <summary>
		/// Gets an icon of a gKey symbol.
		/// </summary>
		public static EditorIcon GKey
		{
			get
			{
				if (gKey == null)
				{
					gKey = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(gKey);
				}
				return gKey;
			}
		}

		/// <summary>
		/// Gets an icon of a globe symbol.
		/// </summary>
		public static EditorIcon Globe
		{
			get
			{
				if (globe == null)
				{
					globe = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(globe);
				}
				return globe;
			}
		}

		/// <summary>
		/// Gets an icon of a grid blocks symbol.
		/// </summary>
		public static EditorIcon GridBlocks
		{
			get
			{
				if (gridBlocks == null)
				{
					gridBlocks = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(gridBlocks);
				}
				return gridBlocks;
			}
		}

		/// <summary>
		/// Gets an icon of a grid image text symbol.
		/// </summary>
		public static EditorIcon GridImageText
		{
			get
			{
				if (gridImageText == null)
				{
					gridImageText = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(gridImageText);
				}
				return gridImageText;
			}
		}

		/// <summary>
		/// Gets an icon of a grid image text list symbol.
		/// </summary>
		public static EditorIcon GridImageTextList
		{
			get
			{
				if (gridImageTextList == null)
				{
					gridImageTextList = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(gridImageTextList);
				}
				return gridImageTextList;
			}
		}

		/// <summary>
		/// Gets an icon of a grid layout symbol.
		/// </summary>
		public static EditorIcon GridLayout
		{
			get
			{
				if (gridLayout == null)
				{
					gridLayout = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(gridLayout);
				}
				return gridLayout;
			}
		}

		/// <summary>
		/// Gets an icon of a hamburger menu symbol.
		/// </summary>
		public static EditorIcon HamburgerMenu
		{
			get
			{
				if (hamburgerMenu == null)
				{
					hamburgerMenu = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(hamburgerMenu);
				}
				return hamburgerMenu;
			}
		}

		/// <summary>
		/// Gets an icon of a house symbol.
		/// </summary>
		public static EditorIcon House
		{
			get
			{
				if (house == null)
				{
					house = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(house);
				}
				return house;
			}
		}

		/// <summary>
		/// Gets an icon of an image symbol.
		/// </summary>
		public static EditorIcon Image
		{
			get
			{
				if (image == null)
				{
					image = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(image);
				}
				return image;
			}
		}

		/// <summary>
		/// Gets an icon of an image collection symbol.
		/// </summary>
		public static EditorIcon ImageCollection
		{
			get
			{
				if (imageCollection == null)
				{
					imageCollection = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(imageCollection);
				}
				return imageCollection;
			}
		}

		/// <summary>
		/// Gets an icon of an info symbol.
		/// </summary>
		public static EditorIcon Info
		{
			get
			{
				if (info == null)
				{
					info = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(info);
				}
				return info;
			}
		}

		/// <summary>
		/// Gets an icon of a letter symbol.
		/// </summary>
		public static EditorIcon Letter
		{
			get
			{
				if (letter == null)
				{
					letter = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(letter);
				}
				return letter;
			}
		}

		/// <summary>
		/// Gets an icon of a light bulb symbol.
		/// </summary>
		public static EditorIcon LightBulb
		{
			get
			{
				if (lightBulb == null)
				{
					lightBulb = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(lightBulb);
				}
				return lightBulb;
			}
		}

		/// <summary>
		/// Gets an icon of a link symbol.
		/// </summary>
		public static EditorIcon Link
		{
			get
			{
				if (link == null)
				{
					link = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(link);
				}
				return link;
			}
		}

		/// <summary>
		/// Gets an icon of a list symbol.
		/// </summary>
		public static EditorIcon List
		{
			get
			{
				if (list == null)
				{
					list = new LazyEditorIcon(34, 34, "iVBORw0KGgoAAAANSUhEUgAAACIAAAAiCAYAAAA6RwvCAAAAe0lEQVRYCe2UQQqAMAwErf//hQ+NBqGHJdBTmz1MQTCIZJhdOiLicji3A0QyAKJJYAQjakBn6448H23ecruf3DOPjZHBFT9D+V9soqlAKKuk1TNW0bSQWINQ1pZO6FLrjijskbkyQlmPqF8tqaJZ/bPlOyCqFSMYUQM6vyOhd+UiNy0nAAAAAElFTkSuQmCC");
					CleanupUtility.DisposeObjectOnAssemblyReload(list);
				}
				return list;
			}
		}

		/// <summary>
		/// Gets an icon of a loading bar symbol.
		/// </summary>
		public static EditorIcon LoadingBar
		{
			get
			{
				if (loadingBar == null)
				{
					loadingBar = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(loadingBar);
				}
				return loadingBar;
			}
		}

		/// <summary>
		/// Gets an icon of a lock locked symbol.
		/// </summary>
		public static EditorIcon LockLocked
		{
			get
			{
				if (lockLocked == null)
				{
					lockLocked = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(lockLocked);
				}
				return lockLocked;
			}
		}

		/// <summary>
		/// Gets an icon of a lock unlocked symbol.
		/// </summary>
		public static EditorIcon LockUnlocked
		{
			get
			{
				if (lockUnlocked == null)
				{
					lockUnlocked = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(lockUnlocked);
				}
				return lockUnlocked;
			}
		}

		/// <summary>
		/// Gets an icon of a lock unlocked symbol. Obsolete; use the correctly spelled LockUnlocked instead.
		/// </summary>
		[Obsolete("Use LockUnlocked instead.", true)]
		[EditorBrowsable(EditorBrowsableState.Never)]
		public static EditorIcon LockUnloacked => LockUnlocked;

		/// <summary>
		/// Gets an icon of a magnifying glass symbol.
		/// </summary>
		public static EditorIcon MagnifyingGlass
		{
			get
			{
				if (magnifyingGlass == null)
				{
					magnifyingGlass = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(magnifyingGlass);
				}
				return magnifyingGlass;
			}
		}

		/// <summary>
		/// Gets an icon of a male symbol.
		/// </summary>
		public static EditorIcon Male
		{
			get
			{
				if (male == null)
				{
					male = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(male);
				}
				return male;
			}
		}

		/// <summary>
		/// Gets an icon of a marker symbol.
		/// </summary>
		public static EditorIcon Marker
		{
			get
			{
				if (marker == null)
				{
					marker = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(marker);
				}
				return marker;
			}
		}

		/// <summary>
		/// Gets an icon of a maximize symbol.
		/// </summary>
		public static EditorIcon Maximize
		{
			get
			{
				if (maximize == null)
				{
					maximize = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(maximize);
				}
				return maximize;
			}
		}

		/// <summary>
		/// Gets an icon of a microphone symbol.
		/// </summary>
		public static EditorIcon Microphone
		{
			get
			{
				if (microphone == null)
				{
					microphone = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(microphone);
				}
				return microphone;
			}
		}

		/// <summary>
		/// Gets an icon of a minimize symbol.
		/// </summary>
		public static EditorIcon Minimize
		{
			get
			{
				if (minimize == null)
				{
					minimize = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(minimize);
				}
				return minimize;
			}
		}

		/// <summary>
		/// Gets an icon of a minus symbol.
		/// </summary>
		public static EditorIcon Minus
		{
			get
			{
				if (minus == null)
				{
					minus = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(minus);
				}
				return minus;
			}
		}

		/// <summary>
		/// Gets an icon of a mobile phone symbol.
		/// </summary>
		public static EditorIcon MobilePhone
		{
			get
			{
				if (mobilePhone == null)
				{
					mobilePhone = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(mobilePhone);
				}
				return mobilePhone;
			}
		}

		/// <summary>
		/// Gets an icon of a money symbol.
		/// </summary>
		public static EditorIcon Money
		{
			get
			{
				if (money == null)
				{
					money = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(money);
				}
				return money;
			}
		}

		/// <summary>
		/// Gets an icon of a move symbol.
		/// </summary>
		public static EditorIcon Move
		{
			get
			{
				if (move == null)
				{
					move = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(move);
				}
				return move;
			}
		}

		/// <summary>
		/// Gets an icon of a multi user symbol.
		/// </summary>
		public static EditorIcon MultiUser
		{
			get
			{
				if (multiUser == null)
				{
					multiUser = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(multiUser);
				}
				return multiUser;
			}
		}

		/// <summary>
		/// Gets an icon of a next symbol.
		/// </summary>
		public static EditorIcon Next
		{
			get
			{
				if (next == null)
				{
					next = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(next);
				}
				return next;
			}
		}

		/// <summary>
		/// Gets an icon of a pacman ghost symbol.
		/// </summary>
		public static EditorIcon PacmanGhost
		{
			get
			{
				if (pacmanGhost == null)
				{
					pacmanGhost = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(pacmanGhost);
				}
				return pacmanGhost;
			}
		}

		/// <summary>
		/// Gets an icon of a paperclip symbol.
		/// </summary>
		public static EditorIcon Paperclip
		{
			get
			{
				if (paperclip == null)
				{
					paperclip = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(paperclip);
				}
				return paperclip;
			}
		}

		/// <summary>
		/// Gets an icon of a pause symbol.
		/// </summary>
		public static EditorIcon Pause
		{
			get
			{
				if (pause == null)
				{
					pause = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(pause);
				}
				return pause;
			}
		}

		/// <summary>
		/// Gets an icon of a pen symbol.
		/// </summary>
		public static EditorIcon Pen
		{
			get
			{
				if (pen == null)
				{
					pen = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(pen);
				}
				return pen;
			}
		}

		/// <summary>
		/// Gets an icon of a pen add symbol.
		/// </summary>
		public static EditorIcon PenAdd
		{
			get
			{
				if (penAdd == null)
				{
					penAdd = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(penAdd);
				}
				return penAdd;
			}
		}

		/// <summary>
		/// Gets an icon of a pen minus symbol.
		/// </summary>
		public static EditorIcon PenMinus
		{
			get
			{
				if (penMinus == null)
				{
					penMinus = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(penMinus);
				}
				return penMinus;
			}
		}

		/// <summary>
		/// Gets an icon of a play symbol.
		/// </summary>
		public static EditorIcon Play
		{
			get
			{
				if (play == null)
				{
					play = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(play);
				}
				return play;
			}
		}

		/// <summary>
		/// Gets an icon of a plus symbol.
		/// </summary>
		public static EditorIcon Plus
		{
			get
			{
				if (plus == null)
				{
					plus = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(plus);
				}
				return plus;
			}
		}

		/// <summary>
		/// Gets an icon of a podium symbol.
		/// </summary>
		public static EditorIcon Podium
		{
			get
			{
				if (podium == null)
				{
					podium = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(podium);
				}
				return podium;
			}
		}

		/// <summary>
		/// Gets an icon of a previous symbol.
		/// </summary>
		public static EditorIcon Previous
		{
			get
			{
				if (previous == null)
				{
					previous = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(previous);
				}
				return previous;
			}
		}

		/// <summary>
		/// Gets an icon of a reception signal symbol.
		/// </summary>
		public static EditorIcon ReceptionSignal
		{
			get
			{
				if (receptionSignal == null)
				{
					receptionSignal = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(receptionSignal);
				}
				return receptionSignal;
			}
		}

		/// <summary>
		/// Gets an icon of a redo symbol.
		/// </summary>
		public static EditorIcon Redo
		{
			get
			{
				if (redo == null)
				{
					redo = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(redo);
				}
				return redo;
			}
		}

		/// <summary>
		/// Gets an icon of a refresh symbol.
		/// </summary>
		public static EditorIcon Refresh
		{
			get
			{
				if (refresh == null)
				{
					refresh = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(refresh);
				}
				return refresh;
			}
		}

		/// <summary>
		/// Gets an icon of a rotate symbol.
		/// </summary>
		public static EditorIcon Rotate
		{
			get
			{
				if (rotate == null)
				{
					rotate = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(rotate);
				}
				return rotate;
			}
		}

		/// <summary>
		/// Gets an icon of a ruler symbol.
		/// </summary>
		public static EditorIcon Ruler
		{
			get
			{
				if (ruler == null)
				{
					ruler = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(ruler);
				}
				return ruler;
			}
		}

		/// <summary>
		/// Gets an icon of a ruler rect symbol.
		/// </summary>
		public static EditorIcon RulerRect
		{
			get
			{
				if (rulerRect == null)
				{
					rulerRect = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(rulerRect);
				}
				return rulerRect;
			}
		}

		/// <summary>
		/// Gets an icon of a settings cog symbol.
		/// </summary>
		public static EditorIcon SettingsCog
		{
			get
			{
				if (settingsCog == null)
				{
					settingsCog = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(settingsCog);
				}
				return settingsCog;
			}
		}

		/// <summary>
		/// Gets an icon of a shopping basket symbol.
		/// </summary>
		public static EditorIcon ShoppingBasket
		{
			get
			{
				if (shoppingBasket == null)
				{
					shoppingBasket = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(shoppingBasket);
				}
				return shoppingBasket;
			}
		}

		/// <summary>
		/// Gets an icon of a shopping cart symbol.
		/// </summary>
		public static EditorIcon ShoppingCart
		{
			get
			{
				if (shoppingCart == null)
				{
					shoppingCart = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(shoppingCart);
				}
				return shoppingCart;
			}
		}

		/// <summary>
		/// Gets an icon of a single user symbol.
		/// </summary>
		public static EditorIcon SingleUser
		{
			get
			{
				if (singleUser == null)
				{
					singleUser = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(singleUser);
				}
				return singleUser;
			}
		}

		/// <summary>
		/// Gets an icon of a smart phone symbol.
		/// </summary>
		public static EditorIcon SmartPhone
		{
			get
			{
				if (smartPhone == null)
				{
					smartPhone = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(smartPhone);
				}
				return smartPhone;
			}
		}

		/// <summary>
		/// Gets an icon of a sound symbol.
		/// </summary>
		public static EditorIcon Sound
		{
			get
			{
				if (sound == null)
				{
					sound = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(sound);
				}
				return sound;
			}
		}

		/// <summary>
		/// Gets an icon of a speech bubble round symbol.
		/// </summary>
		public static EditorIcon SpeechBubbleRound
		{
			get
			{
				if (speechBubbleRound == null)
				{
					speechBubbleRound = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(speechBubbleRound);
				}
				return speechBubbleRound;
			}
		}

		/// <summary>
		/// Gets an icon of a speech bubble square symbol.
		/// </summary>
		public static EditorIcon SpeechBubbleSquare
		{
			get
			{
				if (speechBubbleSquare == null)
				{
					speechBubbleSquare = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(speechBubbleSquare);
				}
				return speechBubbleSquare;
			}
		}

		/// <summary>
		/// Gets an icon of a speech bubbles round symbol.
		/// </summary>
		public static EditorIcon SpeechBubblesRound
		{
			get
			{
				if (speechBubblesRound == null)
				{
					speechBubblesRound = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(speechBubblesRound);
				}
				return speechBubblesRound;
			}
		}

		/// <summary>
		/// Gets an icon of a speech bubbles square symbol.
		/// </summary>
		public static EditorIcon SpeechBubblesSquare
		{
			get
			{
				if (speechBubblesSquare == null)
				{
					speechBubblesSquare = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(speechBubblesSquare);
				}
				return speechBubblesSquare;
			}
		}

		/// <summary>
		/// Gets an icon of a star pointer symbol.
		/// </summary>
		public static EditorIcon StarPointer
		{
			get
			{
				if (starPointer == null)
				{
					starPointer = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(starPointer);
				}
				return starPointer;
			}
		}

		/// <summary>
		/// Gets an icon of a stop symbol.
		/// </summary>
		public static EditorIcon Stop
		{
			get
			{
				if (stop == null)
				{
					stop = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(stop);
				}
				return stop;
			}
		}

		/// <summary>
		/// Gets an icon of a stretch symbol.
		/// </summary>
		public static EditorIcon Stretch
		{
			get
			{
				if (stretch == null)
				{
					stretch = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(stretch);
				}
				return stretch;
			}
		}

		/// <summary>
		/// Gets an icon of a table symbol.
		/// </summary>
		public static EditorIcon Table
		{
			get
			{
				if (table == null)
				{
					table = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(table);
				}
				return table;
			}
		}

		/// <summary>
		/// Gets an icon of a tag symbol.
		/// </summary>
		public static EditorIcon Tag
		{
			get
			{
				if (tag == null)
				{
					tag = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(tag);
				}
				return tag;
			}
		}

		/// <summary>
		/// Gets an icon of a test tube symbol.
		/// </summary>
		public static EditorIcon TestTube
		{
			get
			{
				if (testTube == null)
				{
					testTube = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(testTube);
				}
				return testTube;
			}
		}

		/// <summary>
		/// Gets an icon of a timer symbol.
		/// </summary>
		public static EditorIcon Timer
		{
			get
			{
				if (timer == null)
				{
					timer = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(timer);
				}
				return timer;
			}
		}

		/// <summary>
		/// Gets an icon of a traffic stop light symbol.
		/// </summary>
		public static EditorIcon TrafficStopLight
		{
			get
			{
				if (trafficStopLight == null)
				{
					trafficStopLight = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(trafficStopLight);
				}
				return trafficStopLight;
			}
		}

		/// <summary>
		/// Gets an icon of a transparent symbol.
		/// </summary>
		public static EditorIcon Transparent
		{
			get
			{
				if (transparent == null)
				{
					transparent = new LazyEditorIcon(34, 34, "iVBORw0KGgoAAAANSUhEUgAAACIAAAAiCAYAAAA6RwvCAAAATklEQVRYCe3SsQ0AIAwEMWD/nYNoPQHF0X0Xmdszs35454cj3g0d4k8kkogC7hpJRAF3jSSigLtGElHAXSOJKOCukUQUcNdIIgq4a0SRC9mOA0F8Erf/AAAAAElFTkSuQmCC");
					CleanupUtility.DisposeObjectOnAssemblyReload(transparent);
				}
				return transparent;
			}
		}

		/// <summary>
		/// Gets an icon of a tree symbol.
		/// </summary>
		public static EditorIcon Tree
		{
			get
			{
				if (tree == null)
				{
					tree = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(tree);
				}
				return tree;
			}
		}

		/// <summary>
		/// Gets an icon of a triangle down symbol.
		/// </summary>
		public static EditorIcon TriangleDown
		{
			get
			{
				if (triangleDown == null)
				{
					triangleDown = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(triangleDown);
				}
				return triangleDown;
			}
		}

		/// <summary>
		/// Gets an icon of a triangle left symbol.
		/// </summary>
		public static EditorIcon TriangleLeft
		{
			get
			{
				if (triangleLeft == null)
				{
					triangleLeft = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(triangleLeft);
				}
				return triangleLeft;
			}
		}

		/// <summary>
		/// Gets an icon of a triangle right symbol.
		/// </summary>
		public static EditorIcon TriangleRight
		{
			get
			{
				if (triangleRight == null)
				{
					triangleRight = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(triangleRight);
				}
				return triangleRight;
			}
		}

		/// <summary>
		/// Gets an icon of a triangle up symbol.
		/// </summary>
		public static EditorIcon TriangleUp
		{
			get
			{
				if (triangleUp == null)
				{
					triangleUp = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(triangleUp);
				}
				return triangleUp;
			}
		}

		/// <summary>
		/// Gets an icon of an undo symbol.
		/// </summary>
		public static EditorIcon Undo
		{
			get
			{
				if (undo == null)
				{
					undo = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(undo);
				}
				return undo;
			}
		}

		/// <summary>
		/// Gets an icon of an upload symbol.
		/// </summary>
		public static EditorIcon Upload
		{
			get
			{
				if (upload == null)
				{
					upload = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(upload);
				}
				return upload;
			}
		}

		/// <summary>
		/// Gets an icon of a wifi signal symbol.
		/// </summary>
		public static EditorIcon WifiSignal
		{
			get
			{
				if (wifiSignal == null)
				{
					wifiSignal = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(wifiSignal);
				}
				return wifiSignal;
			}
		}

		/// <summary>
		/// Gets an icon of an x symbol.
		/// </summary>
		public static EditorIcon X
		{
			get
			{
				if (x == null)
				{
					x = new LazyEditorIcon(34, 34, "");
					CleanupUtility.DisposeObjectOnAssemblyReload(x);
				}
				return x;
			}
		}

		/// <summary>
		/// Gets a texture of a test inconclusive symbol.
		/// </summary>
		public static Texture2D TestInconclusive
		{
			get
			{
				if (testInconclusive == null)
				{
					byte[] bytes = Convert.FromBase64String("iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAANElEQVQ4EWP8//8/AyWAiRLNIL1DxIDP58X/gzA27w4RL2BzOkxs4L3AODRSIk3TwcCHAQC4kxR1WozKsgAAAABJRU5ErkJggg==");
					testInconclusive = TextureUtilities.LoadImage(16, 16, bytes);
				}
				return testInconclusive;
			}
		}

		/// <summary>
		/// Gets a texture of a test failed symbol.
		/// </summary>
		public static Texture2D TestFailed
		{
			get
			{
				if (testFailed == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					testFailed = TextureUtilities.LoadImage(16, 16, bytes);
				}
				return testFailed;
			}
		}

		/// <summary>
		/// Gets a texture of a test normal symbol.
		/// </summary>
		public static Texture2D TestNormal
		{
			get
			{
				if (testNormal == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					testNormal = TextureUtilities.LoadImage(16, 16, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(testNormal);
				}
				return testNormal;
			}
		}

		/// <summary>
		/// Gets a texture of a test passed symbol.
		/// </summary>
		public static Texture2D TestPassed
		{
			get
			{
				if (testPassed == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					testPassed = TextureUtilities.LoadImage(16, 16, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(testPassed);
				}
				return testPassed;
			}
		}

		/// <summary>
		/// Gets a texture of a console info icon symbol.
		/// </summary>
		public static Texture2D ConsoleInfoIcon
		{
			get
			{
				if (consoleInfoicon == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					consoleInfoicon = TextureUtilities.LoadImage(32, 32, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(consoleInfoicon);
				}
				return consoleInfoicon;
			}
		}

		/// <summary>
		/// Gets a texture of a console warnicon symbol.
		/// </summary>
		public static Texture2D ConsoleWarnicon
		{
			get
			{
				if (consoleWarnicon == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					consoleWarnicon = TextureUtilities.LoadImage(32, 32, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(consoleWarnicon);
				}
				return consoleWarnicon;
			}
		}

		/// <summary>
		/// Gets a texture of a console error icon symbol.
		/// </summary>
		public static Texture2D ConsoleErroricon
		{
			get
			{
				if (consoleErroricon == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					consoleErroricon = TextureUtilities.LoadImage(32, 32, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(consoleErroricon);
				}
				return consoleErroricon;
			}
		}

		/// <summary>
		/// Gets a texture of an odin inspector logo symbol.
		/// </summary>
		public static Texture2D OdinInspectorLogo
		{
			get
			{
				if (odinInspectorLogo == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					odinInspectorLogo = TextureUtilities.LoadImage(138, 115, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(odinInspectorLogo);
				}
				return odinInspectorLogo;
			}
		}

		/// <summary>
		/// Gets a texture of a scene asset icon symbol.
		/// </summary>
		public static Texture2D UnityLogo
		{
			get
			{
				if (unityLogo == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					unityLogo = TextureUtilities.LoadImage(64, 64, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(unityLogo);
				}
				return unityLogo;
			}
		}

		/// <summary>
		/// Gets an icon representing a GameObject.
		/// </summary>
		public static Texture2D UnityGameObjectIcon
		{
			get
			{
				if (unityGameObjectIcon == null)
				{
					unityGameObjectIcon = EditorGUIUtility.FindTexture("GameObject Icon");
				}
				return unityGameObjectIcon;
			}
		}

		/// <summary>
		/// Gets an icon of a unity info icon.
		/// </summary>
		public static Texture2D UnityInfoIcon
		{
			get
			{
				if (unityInfoIcon == null)
				{
					unityInfoIcon = SdfIcons.CreateTransparentIconTexture(SdfIconType.InfoCircleFill, SirenixGUIStyles.HighlightedTextColor, 20, 20, 2);
					CleanupUtility.DestroyObjectOnAssemblyReload(unityInfoIcon);
				}
				return unityInfoIcon;
			}
		}

		/// <summary>
		/// Gets an icon of a unity warning icon.
		/// </summary>
		public static Texture2D UnityWarningIcon
		{
			get
			{
				if (unityWarningIcon == null)
				{
					unityWarningIcon = SdfIcons.CreateTransparentIconTexture(SdfIconType.ExclamationTriangleFill, SirenixGUIStyles.YellowWarningColor, 20, 20, 2);
					CleanupUtility.DestroyObjectOnAssemblyReload(unityWarningIcon);
				}
				return unityWarningIcon;
			}
		}

		/// <summary>
		/// Gets an icon of a unity error icon.
		/// </summary>
		public static Texture2D UnityErrorIcon
		{
			get
			{
				if (unityErrorIcon == null)
				{
					unityErrorIcon = SdfIcons.CreateTransparentIconTexture(SdfIconType.ExclamationOctagonFill, SirenixGUIStyles.RedErrorColor, 20, 20, 2);
					CleanupUtility.DestroyObjectOnAssemblyReload(unityErrorIcon);
				}
				return unityErrorIcon;
			}
		}

		/// <summary>
		/// Gets an icon of a unity folder.
		/// </summary>
		public static Texture2D UnityFolderIcon
		{
			get
			{
				if (unityFolderIcon == null)
				{
					unityFolderIcon = AssetPreview.GetMiniThumbnail(AssetDatabase.LoadAssetAtPath("Assets", typeof(UnityEngine.Object)));
				}
				return unityFolderIcon;
			}
		}

		/// <summary>
		/// Gets the dark mode version of the ObjectFieldButton texture.
		/// </summary>
		public static Texture2D ObjectFieldButtonDark
		{
			get
			{
				if (objectFieldButtonDark == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					objectFieldButtonDark = TextureUtilities.LoadImage(24, 24, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(objectFieldButtonDark);
				}
				return objectFieldButtonDark;
			}
		}

		/// <summary>
		/// Gets the light mode version of the ObjectFieldButton texture.
		/// </summary>
		public static Texture2D ObjectFieldButtonLight
		{
			get
			{
				if (objectFieldButtonLight == null)
				{
					byte[] bytes = Convert.FromBase64String("");
					objectFieldButtonLight = TextureUtilities.LoadImage(24, 24, bytes);
					CleanupUtility.DestroyObjectOnAssemblyReload(objectFieldButtonLight);
				}
				return objectFieldButtonLight;
			}
		}

		/// <summary>
		/// Gets the ObjectFieldButton texture based on the current active theme.
		/// </summary>
		public static Texture2D ObjectFieldButton
		{
			get
			{
				if (!EditorGUIUtility.isProSkin)
				{
					return ObjectFieldButtonLight;
				}
				return ObjectFieldButtonDark;
			}
		}
	}
}
