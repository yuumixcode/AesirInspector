	...
	public abstract class OdinValueDrawer<T> : OdinDrawer
	{
		...
		public IPropertyValueEntry<T> ValueEntry
		{
			get
			{
				if (valueEntry == null)
				{
					valueEntry = base.Property.TryGetTypedValueEntry<T>();
					if (valueEntry == null)
					{
		...
		protected override void DrawPropertyLayout(GUIContent label)
		{
			if (label != null)
			{
				EditorGUILayout.LabelField(label, new GUIContent("The DrawPropertyLayout method has not been implemented for the drawer of type '" + GetType().GetNiceName() + "', or you are calling base.DrawPropertyLayout(label)."));
			}
			else
			{
				EditorGUILayout.LabelField(new GUIContent("The DrawPropertyLayout method has not been implemented for the drawer of type '" + GetType().GetNiceName() + "', or you are calling base.DrawPropertyLayout(label)."));
			}
		}
		...
		public sealed override bool CanDrawProperty(InspectorProperty property)
		{
			...
			if (entry != null && CanDrawTypeFilter(entry.TypeOfValue))
			{
