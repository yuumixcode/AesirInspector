...
namespace Sirenix.OdinInspector.Editor.Drawers
{
	...
	public sealed class PropertyRangeAttributeFloatDrawer : OdinAttributeDrawer<PropertyRangeAttribute, float>
	{
		...
		{
			float min = ((getterMinValue != null) ? getterMinValue.GetValue() : ((float)base.Attribute.Min));
			float max = ((getterMaxValue != null) ? getterMaxValue.GetValue() : ((float)base.Attribute.Max));
			...
			float value = SirenixEditorFields.RangeFloatField(label, base.ValueEntry.SmartValue, Mathf.Min(min, max), Mathf.Max(min, max));
