...
namespace Sirenix.Utilities
{
	...
	public static class MultiDimArrayUtilities
	{
		...
		public static TElement[,] InsertOneColumnLeft<TElement>(TElement[,] array, int columnIndex)
		{
			...
			TElement[,] newArr = new TElement[colCount + 1, Math.Max(rowCount, 1)];
		...
		{
			int colCount = arr.GetLength(0);
			int rowCount = arr.GetLength(1);
			TElement[,] newArr = new TElement[colCount + 1, Math.Max(rowCount, 1)];
			for (int x = 0; x < colCount; x++)
			{
				int tmpX = x;
		...
		public static TElement[,] InsertOneRowAbove<TElement>(TElement[,] array, int rowIndex)
		{
			int colCount = array.GetLength(0);
			int rowCount = array.GetLength(1);
			TElement[,] newArr = new TElement[Math.Max(colCount, 1), rowCount + 1];
			for (int y = 0; y < rowCount; y++)
			{
				int tmpY = y;
				if (tmpY >= rowIndex)
		...
		public static TElement[,] DuplicateColumn<TElement>(TElement[,] array, int columnIndex)
		{
			...
			{
				newArr[columnIndex, i] = array[columnIndex, i];
			}
			return newArr;
		}
		...
		public static TElement[,] DuplicateRow<TElement>(TElement[,] array, int rowIndex)
		{
			...
			{
				newArr[i, rowIndex] = array[i, rowIndex];
			}
			return newArr;
		}
		...
		public static TElement[,] MoveColumn<TElement>(TElement[,] array, int fromColumn, int toColumn)
		{
			...
			int colCount = array.GetLength(0);
			int rowCount = array.GetLength(1);
			TElement[,] newArr = new TElement[colCount, rowCount];
			if (fromColumn < toColumn)
			{
				for (int x = 0; x < colCount; x++)
				{
			...
			else
			{
				...
				{
					int offset2 = ((i > toColumn && i <= fromColumn) ? 1 : 0);
					for (int j = 0; j < rowCount; j++)
					{
						if (i == toColumn + 1)
						{
							newArr[i, j] = array[fromColumn, j];
						}
						else
						{
							newArr[i, j] = array[i - offset2, j];
						}
					}
				}
			}
		...
		public static TElement[,] MoveRow<TElement>(TElement[,] array, int fromRow, int toRow)
		{
			...
			else
			{
				...
				{
					int offset2 = ((i > toRow && i <= fromRow) ? 1 : 0);
					for (int j = 0; j < colCount; j++)
					{
						if (i == toRow + 1)
						{
							newArr[j, i] = array[j, fromRow];
						}
						else
						{
							newArr[j, i] = array[j, i - offset2];
						}
					}
				}
			}
		...
		{
			int colCount = array.GetLength(0) - 1;
			int rowCount = array.GetLength(1);
			if (colCount <= 0)
			{
				colCount = 0;
		...
		{
			int colCount = array.GetLength(0);
			int rowCount = array.GetLength(1) - 1;
			if (rowCount <= 0)
			{
				colCount = 0;
				rowCount = 0;
